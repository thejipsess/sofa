...
plot_proba_truth <- function(x, test = NULL, rec = NULL, type = "scatter",
  ...
  if(save)
    ggsave(p, filename = paste("Figures/", save_name, ".svg", sep = ""))
...
plot_radar <- function(model, test, dep_var, rec = NULL, model_name = "model",
  ...
  add_metrics <- function(model, model_name, test, df, i){
    ...
    } else if(class(model)[1] == "lrm") {
      df_test_pred <- as.factor(ifelse(
        predict(model, test, type = "fitted") < 0.5, 0, 1))
      df_test_pred_proba <- 1 - predict(model, test, type = "fitted")
